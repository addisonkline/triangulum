...
def normals(d1, d2, d3, dfs, elev, lapse): # d1, d2, d3 distances, dfs a list of the corresponding dataframes
...
def occurrence_probabilities(df):
    ...
    prob_max_geq95 = 1 - np.power(norm.cdf(95, df.iloc[0], daily_max_std), days)
    prob_max_geq86 = 1 - np.power(norm.cdf(86, df.iloc[0], daily_max_std), days)
    prob_max_geq68 = 1 - np.power(norm.cdf(68, df.iloc[0], daily_max_std), days)
    prob_max_geq50 = 1 - np.power(norm.cdf(50, df.iloc[0], daily_max_std), days)
    # probabilities of occurrences: min less than or equal to stated temp
    prob_min_leq50 = 1 - np.power((1 - norm.cdf(50, df.iloc[2], daily_min_std)), days)
    prob_min_leq32 = 1 - np.power((1 - norm.cdf(32, df.iloc[2], daily_min_std)), days)
    prob_min_leq14 = 1 - np.power((1 - norm.cdf(14, df.iloc[2], daily_min_std)), days)
    prob_min_leq5 = 1 - np.power((1 - norm.cdf(5, df.iloc[2], daily_min_std)), days)
